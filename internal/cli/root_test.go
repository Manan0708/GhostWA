...
func TestRunNoArgsPrintsWACLI(t *testing.T) {
	...
	if got := strings.TrimSpace(stdout.String()); got != "GhostWA v2.5.3 — Silent, High-Performance WhatsApp Terminal Client" {
		t.Fatalf("stdout = %q, want %q", got, "GhostWA v2.5.3 — Silent, High-Performance WhatsApp Terminal Client")
	}
	if stderr.Len() != 0 {
