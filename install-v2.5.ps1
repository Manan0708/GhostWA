# GhostWA Dev v2.5.7 Automated One-Line Installer for Windows (PowerShell)
...
Write-Host "[1/3] Downloading GhostWA v2.5.7 pre-compiled executable..." -ForegroundColor Yellow
$urlGhostWA = "https://raw.githubusercontent.com/Manan0708/GhostWA/main/bin/ghostwa-v2.5.7.exe"
...
if (!(Test-Path $ghostwaPath)) {
    $localGhostWA = Join-Path $PSScriptRoot "bin\ghostwa-v2.5.7.exe"
...
if (!(Test-Path $ghostwaPath)) {
    Write-Host "[-] Installation failed. Could not locate or download ghostwa-v2.5.7.exe executable." -ForegroundColor Red
    exit 1
}
