# GhostWA Dev v2.5.3 Automated One-Line Installer for Windows (PowerShell)
# Usage: irm https://raw.githubusercontent.com/Manan0708/GhostWA/main/install-v2.5.ps1 | iex

[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12

Write-Host ""
Write-Host "  ┌────────────────────────────────────────────────────────┐" -ForegroundColor Cyan
Write-Host "  │   ⚡ GhostWA v2.5.1 (Dev Release) - Auto-Installer    │" -ForegroundColor Cyan
Write-Host "  └────────────────────────────────────────────────────────┘" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[1/3] Downloading GhostWA v2.5.3 pre-compiled executable..." -ForegroundColor Yellow
$urlGhostWA = "https://raw.githubusercontent.com/Manan0708/GhostWA/main/bin/ghostwa-v2.5.3.exe"
...
if (!(Test-Path $ghostwaPath)) {
    $localGhostWA = Join-Path $PSScriptRoot "bin\ghostwa-v2.5.3.exe"
...
if (!(Test-Path $ghostwaPath)) {
    Write-Host "[-] Installation failed. Could not locate or download ghostwa-v2.5.3.exe executable." -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "  ✅ GhostWA v2.5.1 (Dev) Installed Successfully!" -ForegroundColor Green
Write-Host "  ------------------------------------------------------------" -ForegroundColor Gray
Write-Host "  To start using GhostWA v2.5.1:" -ForegroundColor White
