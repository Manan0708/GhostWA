# GhostWA Dev v2.5.5 Automated One-Line Installer for Windows (PowerShell)
...
Write-Host "[1/3] Downloading GhostWA v2.5.5 pre-compiled executable..." -ForegroundColor Yellow
$urlGhostWA = "https://raw.githubusercontent.com/Manan0708/GhostWA/main/bin/ghostwa-v2.5.5.exe"
...
if (!(Test-Path $ghostwaPath)) {
    $localGhostWA = Join-Path $PSScriptRoot "bin\ghostwa-v2.5.5.exe"
...
if (!(Test-Path $ghostwaPath)) {
    Write-Host "[-] Installation failed. Could not locate or download ghostwa-v2.5.5.exe executable." -ForegroundColor Red
    exit 1
}
